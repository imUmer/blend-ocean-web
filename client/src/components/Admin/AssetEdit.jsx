...
import { fetchAssetById, updateAssetById } from "../../services/adminService";
...
const AssetEdit = () => {
  ...
  useEffect(() => {
    const fetchAssetDetails = async () => {
      try {
        setLoading(true);
        const asset = await fetchAssetById(token, id); // Fetch asset details by ID
        setFormData({
          ...asset,
          exportFormats: asset.exportFormats.map((f) => f.trim()), // Convert array to comma-separated string
        });
        loadImages(asset?.assetImagesId)
        setParent(asset?.type)
        setSubParent(asset?.category)
        setLoading(false);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const saveImage = async ({ base64Images }) => {
        try {
          ...
          const docId = formData?.assetImagesId;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // const response = await updateAssetById(id, { ...formData, data });
      navigate("/admin");
